...
│
├── UI/
│   ├── InventoryGridUI.cs       # 【现有·迁移】网格生成
│   ├── InventorySlot.cs         # 【现有 Slot.cs·迁移+改名】
│   ├── ItemView.cs              # 物品视觉（后续）
...
> ⚠️ 现有 `Scripts/Inventory/` 目录中的 `InventoryGridUI.cs`、`Slot.cs` 需迁移到 `UI/`，`Slot.cs` 改名 `InventorySlot.cs`。**迁移必须在 Unity 编辑器内拖拽 + F2 重命名完成**（Unity 自动维护 `.meta` 的 GUID），否则 `ItemSlot.prefab` 与 `Test.unity` 中的引用会变 `Missing`。
...
4. ⏳ 迁移现有 3 脚本加命名空间（本次仅加 namespace，物理移动在 Unity 编辑器内完成）
