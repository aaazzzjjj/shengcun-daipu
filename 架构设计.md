...
> 状态：已定稿（第一版）
> 日期：2026-09-20
> 依据：`策划文档.md` 第七章「数据架构」的改良 MVC，结合当前项目现状（`InventoryConfigSO` + `InventoryGridUI` + `InventorySlot` 已打通 24×10 网格生成）落地为可执行的目录与依赖规范。
...
| 配置数据 | **ScriptableObject（静态档案）** | 与现有 `InventoryConfigSO` 风格一致，策划可编辑、可引用 icon/prefab |
...
│
├── Config/
│   ├── InventoryConfigSO.cs    # 【现有·保留】网格布局 SO
...
| `Scripts/Config/InventoryConfigSO.cs` | 原地保留 | 加 `namespace SurvivalPawnShop.Config` |
